...
def get_position(name: str, reference_database: geopandas.GeoDataFrame):
    ...
    if relevant_results.empty:
        relevant_results = reference_database.loc[reference_database["name"].str.contains(name, na=False)]
    if relevant_results.empty:
        relevant_results = geopandas.GeoDataFrame().reindex_like(reference_database)
        relevant_results.crs = reference_database.crs
...
def get_map(data: pd.DataFrame):
    ...
    leaflet_map.fit_bounds(bounding_box)
    return leaflet_map