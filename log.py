...
from value_replacement import replacement
...
def parse_events(events_raw: pd.Series) -> pd.DataFrame:
    ...
    return result.astype({OBU: pd.Int64Dtype(), RBC_ID: pd.Int64Dtype()})
...
def list_train_numbers(data: pd.Series):
...
def list_engines(data: pd.Series):
    return pd.DataFrame(sorted(data.unique()), columns=[OBU])


def is_connection_loss(data) -> pd.Series:
    return (data[EOM]) & (data[RBC_ID] == data[NEXT_RBC]) & \
        (data[PREV_MODE].isin([FS, OS]) & (data[NEXT_MODE].isin([SR])))


def is_trip(data) -> pd.Series:
    return (data[MODE] == TR) & (data[PREV_MODE] != TR)


def is_post_trip(data) -> pd.Series:
    return (data[MODE] == PT) & (~data[PREV_MODE].isin([TR, PT]))


def is_system_failure(data) -> pd.Series:
    return (data[MODE] == SF) & (data[PREV_MODE] != SF)


def is_isolation(data) -> pd.Series:
    return (data[MODE] == IS) & (~data[PREV_MODE].isin([IS, TR, PT]))


def analyse(file_name: str) -> pd.DataFrame:
    data = load_data(file_name)
    return analyse_from_data(data)


def analyse_from_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data[INCIDENT_NO_IS] = is_trip(data) | is_post_trip(data) | is_system_failure(data) | is_connection_loss(data)
    data[INCIDENT] = is_trip(data) | is_post_trip(data) | is_isolation(data) | is_system_failure(data) | \
        is_connection_loss(data)
